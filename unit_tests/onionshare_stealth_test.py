...
class OnionShareGuiTest(unittest.TestCase):
    ...
    def test_copy_hidserv_auth_button_click_shows_message(self):
        '''Test that the Copy HidservAuth button shows a status message when clicked'''
        QtTest.QTest.mouseClick(self.gui.server_status.copy_hidservauth_button, QtCore.Qt.LeftButton)
        self.assertEqual(self.gui.status_bar.currentMessage(), strings._('gui_copied_hidservauth', True))

    @pytest.mark.run(order=6)
    def test_copy_hidserv_auth_button_click_clears_message(self):
        '''Test that the Copy HidservAuth message disappears after 2 seconds'''
